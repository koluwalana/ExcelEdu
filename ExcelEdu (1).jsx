...
export default function ExcelEdu() {
  const [screen, setScreen] = useState("landing"); // landing|home|quiz|mock|chat|paywall|success
  ...
  const MOCK_DURATION = 30 * 60; // 30 mins
  const MOCK_COUNT = 10;
  ...
  const handlePayment = () => {
    ...
    loadPaystack(emailInput.trim(), (ref) => {
      setIsPaid(true);
      setScreen("success");
    });
  };
  ...
  if (screen === "landing") return (
    <div style={{ minHeight: "100vh", background: `linear-gradient(160deg, #e8f5e9 0%, #f9fbe7 60%, #e3f2fd 100%)`, fontFamily: "'Nunito', 'Segoe UI', sans-serif", overflowX: "hidden" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .cta-btn { transition: transform 0.15s; }
      ...
      <nav style={{ padding: "16px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(255,255,255,0.7)", backdropFilter: "blur(12px)", position: "sticky", top: 0, zIndex: 100, borderBottom: "1px solid rgba(0,200,83,0.15)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={{ fontFamily: "'Fredoka One', cursive", fontSize: 20, color: "#00922e", letterSpacing: 0.5 }}>ExcelEdu</span>
        </div>
      ...
      <div style={{ textAlign: "center", padding: "56px 24px 40px", animation: "slideUp 0.6s ease" }}>
        ...
        <h1 style={{ fontFamily: "'Fredoka One', cursive", fontSize: "clamp(32px,8vw,52px)", color: "#00922e", lineHeight: 1.15, marginBottom: 16 }}>
          Pass JAMB & WAEC<br />
          <span style={{ color: "#ff6d00" }}>First Time. Every Time.</span>
        </h1>
        ...
        <button onClick={() => setScreen("home")} className="cta-btn" style={{ background: "linear-gradient(135deg, #00c853, #00e676)", color: "#fff", border: "none", borderRadius: 16, padding: "16px 40px", fontWeight: 900, fontSize: 18, cursor: "pointer", boxShadow: "0 8px 24px rgba(0,200,83,0.4)", fontFamily: "'Nunito', sans-serif" }}>
      ...
      <div style={{ padding: "0 20px 48px", maxWidth: 500, margin: "0 auto" }}>
        ...
        <div style={{ marginTop: 28, background: "linear-gradient(135deg, #00c853 0%, #00e676 100%)", borderRadius: 20, padding: "28px 24px", textAlign: "center", boxShadow: "0 12px 32px rgba(0,200,83,0.35)", color: "#fff" }}>
          ...
          <div style={{ fontSize: 14, opacity: 0.85, marginBottom: 20, fontWeight: 600 }}>Unlimited access to everything · Cancel anytime</div>
      ...
      <div style={{ maxWidth: 400, width: "100%" }}>
        ...
        <div style={{ background: "#fff", borderRadius: 20, padding: 24, boxShadow: "0 8px 24px rgba(0,0,0,0.08)", marginBottom: 16 }}>
          ...
          <input
            ...
            style={{ width: "100%", padding: "12px 14px", borderRadius: 10, border: "2px solid #c8e6c9", fontSize: 15, outline: "none", fontFamily: "'Nunito', sans-serif", color: "#0d2b1a" }}
          />
  ...
  // ════════════════════════════════════════════════════════════════════
  // SHARED WRAPPER (home, quiz, mock, chat)
  // ════════════════════════════════════════════════════════════════════
  return (
    <div style={{ minHeight: "100vh", background: "#f0faf4", fontFamily: "'Nunito', 'Segoe UI', sans-serif", maxWidth: 480, margin: "0 auto" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Nunito:wght@400;600;700;800;900&family=Fredoka+One&display=swap');
        * { box-sizing: border-box; }
        ...
        .opt-btn:hover { filter: brightness(0.96); }
        .tab-btn:hover { background: rgba(0,200,83,0.1) !important; }
      `}</style>

      {/* HEADER */}
      <div style={{ background: "linear-gradient(135deg, #00c853 0%, #00e676 100%)", padding: "14px 20px 0", borderBottomLeftRadius: 0, borderBottomRightRadius: 0, boxShadow: "0 4px 20px rgba(0,200,83,0.3)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span style={{ fontFamily: "'Fredoka One', cursive", fontSize: 18, color: "#fff", letterSpacing: 0.5 }}>ExcelEdu</span>
          </div>
          <div style={{ display: "flex", gap: 14 }}>
            ...
            {!isPaid && (
              <div style={{ background: "rgba(255,255,255,0.2)", borderRadius: 12, padding: "2px 10px", fontSize: 11, color: "#fff", fontWeight: 700, alignSelf: "center" }}>
                {Math.max(0, FREE_QUESTIONS_LIMIT - freeUsed)} free left
              </div>
            )}
            {isPaid && <div style={{ background: "rgba(255,255,255,0.2)", borderRadius: 12, padding: "2px 10px", fontSize: 11, color: "#fff", fontWeight: 700, alignSelf: "center" }}>✓ Pro</div>}
          </div>
        </div>
      ...
      {screen === "home" && (
        <div style={{ padding: "24px 20px", animation: "fadeUp 0.4s ease" }}>
          <h2 style={{ fontFamily: "'Fredoka One', cursive", fontSize: 24, color: "#00922e", marginBottom: 6 }}>
            Hello, Scholar! 👋
          </h2>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              {EXAMS.map(e => (
                <button key={e} onClick={() => setExam(e)} style={{
                  ...
                  border: exam === e ? "2px solid #00c853" : "2px solid #ddd",
                  background: exam === e ? "#e8faf0" : "#fff",
                  color: exam === e ? "#00922e" : "#888",
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {SUBJECTS.map(s => (
                <button key={s} onClick={() => setSubject(s)} style={{
                  ...
                  border: subject === s ? "2px solid #00c853" : "2px solid transparent",
                  background: subject === s ? "#e8faf0" : "#fff",
                  color: subject === s ? "#00922e" : "#444",
                  fontSize: 13, fontWeight: subject === s ? 800 : 600, cursor: "pointer",
      ...
      {screen === "quiz" && (
        <div style={{ padding: "20px", animation: "fadeUp 0.3s ease" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
            ...
            <div style={{ background: "#e8faf0", border: "1px solid #00c853", borderRadius: 20, padding: "4px 14px", fontSize: 12, color: "#00922e", fontWeight: 800 }}>
              {exam} · {subject}
            </div>
                ...
                {Object.entries(question.options).map(([opt, text]) => {
                  ...
                  return (
                    <button key={opt} className="opt-btn" onClick={() => handleAnswer(opt)} style={{
                      ...
                      fontSize: 14, cursor: selected ? "default" : "pointer",
                      display: "flex", gap: 12, alignItems: "flex-start",
                      transition: "all 0.2s", fontFamily: "'Nunito', sans-serif", fontWeight: 700,
                      boxShadow: "0 2px 8px rgba(0,0,0,0.05)",
                    }}>
                      <span style={{ minWidth: 28, height: 28, borderRadius: 8, background: opt === question.answer && selected ? "#00c853" : opt === selected && selected ? "#ff1744" : "#f0f4f0", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 900, color: (opt === question.answer || opt === selected) && selected ? "#fff" : "#666" }}>{opt}</span>
        ...
        <div style={{ padding: "20px", animation: "fadeUp 0.3s ease" }}>
          ...
          {!mockLoading && mockDone && (
            <div style={{ textAlign: "center", padding: "32px 20px", animation: "fadeUp 0.4s ease" }}>
              <div style={{ fontSize: 64, marginBottom: 16 }}>{mockScore >= 7 ? "🏆" : mockScore >= 5 ? "👍" : "📚"}</div>
              ...
              <div style={{ fontSize: 16, color: "#6b8f76", marginBottom: 8, fontWeight: 700 }}>
                {Math.round((mockScore / mockQuestions.length) * 100)}% · {mockScore >= 7 ? "Excellent work! 🔥" : mockScore >= 5 ? "Good effort! Keep going 💪" : "Keep practising! You'll get there 📖"}
              </div>
                ...
                {mockQuestions.map((q, i) => (
                  <div key={i} style={{ background: "#fff", borderRadius: 12, padding: "12px 16px", textAlign: "left", border: `2px solid ${mockAnswers[i] === q.answer ? "#00c853" : "#ff6060"}`, boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: mockAnswers[i] === q.answer ? "#00922e" : "#cc0000", marginBottom: 4 }}>
                      Q{i + 1} · {mockAnswers[i] === q.answer ? "✅ Correct" : `❌ You answered ${mockAnswers[i] || "—"}, Answer: ${q.answer}`}
                    </div>
                    <div style={{ fontSize: 12, color: "#444", lineHeight: 1.5, fontWeight: 600 }}>{q.question.substring(0, 80)}...</div>
                  </div>
                ))}
              </div>
              <button onClick={startMock} style={{ marginTop: 20, width: "100%", padding: "14px", borderRadius: 12, background: "linear-gradient(135deg,#00c853,#00e676)", color: "#fff", fontWeight: 900, fontSize: 15, border: "none", cursor: "pointer", fontFamily: "'Nunito', sans-serif" }}>
                Try Another Mock →
              </button>
            </div>
          )}
          ...
          {!mockLoading && !mockDone && mockQuestions.length > 0 && (
            <div>
              {/* Progress bar */}
              <div style={{ background: "#e8f5e9", borderRadius: 8, height: 6, marginBottom: 20, overflow: "hidden" }}>
                ...
                <div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
                    {Object.entries(mockQuestions[mockCurrent].options).map(([opt, text]) => (
                      <button key={opt} onClick={() => setMockAnswers(a => ({ ...a, [mockCurrent]: opt }))} style={{
                        padding: "12px 16px", borderRadius: 10, textAlign: "left",
                        background: mockAnswers[mockCurrent] === opt ? "#e8faf0" : "#fff",
                        border: `2px solid ${mockAnswers[mockCurrent] === opt ? "#00c853" : "#eee"}`,
                        color: mockAnswers[mockCurrent] === opt ? "#00922e" : "#444",
                        fontSize: 14, cursor: "pointer", display: "flex", gap: 10,
                        fontFamily: "'Nunito', sans-serif", fontWeight: 700,
                        transition: "all 0.15s",
                      }}>
                        <span style={{ minWidth: 26, height: 26, borderRadius: 6, background: mockAnswers[mockCurrent] === opt ? "#00c853" : "#f0f4f0", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 900, color: mockAnswers[mockCurrent] === opt ? "#fff" : "#666" }}>{opt}</span>
                        {text}
                      </button>
                    ))}
                  </div>
                  ...
                  <div style={{ display: "flex", gap: 10 }}>
                    {mockCurrent < mockQuestions.length - 1 ? (
                      <button onClick={() => setMockCurrent(c => c + 1)} style={{ flex: 1, padding: "12px", borderRadius: 10, background: "linear-gradient(135deg,#00c853,#00e676)", color: "#fff", fontWeight: 800, fontSize: 15, border: "none", cursor: "pointer", fontFamily: "'Nunito', sans-serif" }}>
                        Next →
                      </button>
                    ) : (
                      <button onClick={submitMock} style={{ flex: 1, padding: "12px", borderRadius: 10, background: "linear-gradient(135deg,#ff6d00,#ff9100)", color: "#fff", fontWeight: 800, fontSize: 15, border: "none", cursor: "pointer", fontFamily: "'Nunito', sans-serif" }}>
                        Submit Exam ✓
                      </button>
                    )}
                  </div>
      ...
      {screen === "chat" && (
        <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 118px)" }}>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px", display: "flex", flexDirection: "column", gap: 10, background: "#f8fdf9" }}>
            {chatMessages.map((m, i) => (
              <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start", animation: "fadeUp 0.2s ease" }}>
                <div style={{
                  ...
                  background: m.role === "user" ? "linear-gradient(135deg, #00c853, #00e676)" : "#fff",
                  color: m.role === "user" ? "#fff" : "#0d2b1a",
                  fontSize: 14, lineHeight: 1.7, fontWeight: 600,
                  boxShadow: "0 2px 10px rgba(0,0,0,0.08)",
                  border: m.role !== "user" ? "1px solid #e8f5e9" : "none",
                }}>
                  {m.content}
                </div>
              </div>
          ...
          <div style={{ padding: "12px 16px", borderTop: "1px solid #e8f5e9", background: "#fff" }}>
            <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
              <textarea
                ...
                placeholder="Ask about any topic or past question..."
                rows={2}
